...
if __name__ == "__main__":
    ...
    OutputName = "Results/Tree/Tree_HIT"+str(ooo)+"_pitch"+str(pitch)+".root"
    ...
    #csv files with coordinates after clustering procedure(traditional method)
    OutputNameCSV_x = "TrkData_HIT_viewx_afterCL_"+str(ooo)+"_pitch"+str(pitch)+".csv"
    OutputNameCSV_x = folder_path_csv+OutputNameCSV_x
    print('OutputNameCSV_x ',OutputNameCSV_x)
    OutputNameCSV_y = "TrkData_HIT_viewy_afterCL_"+str(ooo)+"_pitch"+str(pitch)+".csv"
    OutputNameCSV_y = folder_path_csv+OutputNameCSV_y
    print('OutputNameCSV_y ',OutputNameCSV_y)
    
    #csv files with coordinates before clustering procedure(traditional method)
    OutputNameCSV_x_nocl = "TrkData_HIT_viewx_raw_"+str(ooo)+"_pitch"+str(pitch)+".csv"
    OutputNameCSV_x_nocl = folder_path_csv+OutputNameCSV_x_nocl
    print('OutputNameCSV_x_nocl ',OutputNameCSV_x_nocl)
    OutputNameCSV_y_nocl = "TrkData_HIT_viewy_raw_"+str(ooo)+"_pitch"+str(pitch)+".csv"
    ...
    for i in range(nhit):
        ...
        particle_id_vec   = tree.ParticleID
    ...
    #list of objects per hit
        ...
        for j in range(len(fiber_vec)):
            ...
            if (iv == 0): 
                ...
                #particle_id[il][iv].append(particle_id_vec[j])
               # print('******viewx*********',' il ',il,' iv ',iv,' fiberj ',j,' particle_id_vec ',particle_id_vec[j],' cfib ',cfib  )
            ...
            if (iv == 1): 
                ...
                #particle_id[il][iv].append(particle_id_vec[j])
                #print('*******viewy********',' il ',il,' iv ',iv,' particle_id_vec ',particle_id_vec[j])
        ...
        for il in range(totLayers):
            ...
            for iv in range(nview):
                ...
                for ind in range (len(ordered_index_list[il][iv])):
                    ...
                    ordered_particleid[il][iv].append(particle_id[il][iv][ordered_index_list[il][iv][ind]])
              #  if(iv==0):
               #      print("STRIP:",Strips[il][iv]) 
                ...
                #     print(ordered_particleid[il][iv])
        ...
        PEStrips = ordered_PE 
        particle_id = ordered_particleid
        ...
        for il in range(totLayers):
            for iv in range(nview):
                if(iv==0):
                    raw_x_noclu.append([i,il,iv,Strip_r[il][iv],np.mean(z[il][iv]),PEStrips[il][iv],particle_id[il][iv]])
                    #print('****PE strip ',i,' il ',il,' iv ',iv ,' ' , PEStrips[il][iv],' ',Strip_r[il][iv],' ',particle_id[il][iv])
                elif (iv==1):
                    raw_y_noclu.append([i,il,iv,Strip_r[il][iv],np.mean(z[il][iv]),PEStrips[il][iv],particle_id[il][iv]])
        ...
        for il in range(totLayers):
            ...
            for iv in range(nview):
                ...
                if eval(sTclu)[0] > 0 :
                    ...
                    x0_1 = idx_cl[il][iv][0]
                    x0_2 = idx_cl[il][iv][-1]
                    # print('idx ',idx_cl[il][iv],'x0_1 ',x0_1, ' x0_2  ',x0_2)
                                        
                    indices_cl = [index for index, element in enumerate(Strips[il][iv]) if element in idx_cl[il][iv]]
                    Strips_cl[il][iv].append(Strip_r[il][iv][indices_cl[0]:indices_cl[-1]+1])                                         
                    # if(len(indices_cl)!=len(Strips[il][iv])):
                    #     print('ev ',i,' ly ',il,' iv ',iv,' indices_cl ',indices_cl, ' ',len(indices_cl),' ',len(Strips[il][iv]) )
                    #     print('old strip before clustering ',Strip_r[il][iv] )
                    #     print('new strip ',Strip_r[il][iv][indices_cl[0]:indices_cl[-1]+1])
                    ...
                    if den >= 3:
                        ...
                        if iv==0:
                            ...
                            # print('###after clustering ev ', i, ' layers ', il, ' view ', iv, ' xrec ',
                                # bc[il][iv],' zx_rec ',String_zs[il][iv],' xtrack ',primary_track[il][iv], 'zxtrack ',String_zs[il][iv])
                            raw_x.append([i,il,iv, bc[il][iv],String_zs[il][iv],debc[il][iv],2*rfiber,xMC_vec[il],String_zs[il][iv]])
                        elif iv==1:
                            ...
                            #print('###after clustering ev ', i, ' layers ', il, ' view ', iv, ' xrec ',
                                #bc[il][iv],' zx_rec ',String_zs[il][iv],' xtrack ',primary_track[il][iv], 'zxtrack ',String_zs[il][iv])
                            raw_y.append([i,il,iv, bc[il][iv],String_zs[il][iv],debc[il][iv],2*rfiber,yMC_vec[il],String_zs[il][iv]])
   ...
   # print('r2_x',r2_score(truex,predx))
    ...
    #csv file of the position with the charged weighted position
    ...
    data_trackingx_nocl=pd.DataFrame(raw_x_noclu,columns=['Ev','ly','view','x_hit','zx_hit','PE','PID'])
    data_trackingx_nocl['xMC']=data_trackingx_cl['xMC']
    data_trackingy_nocl=pd.DataFrame(raw_y_noclu,columns=['Ev','ly','view','y_hit','zy_hit','PE','PID'])
