...
def evaluateRecoCosineDirector(Mx,My):#M angolar coefficient x=a1*z+b1;y=a2*z+b2
    a0 = pow(Mx,2.)
    a1 = pow(My,2.)
...
#running command
#python Script/CluTrack.py /lustrehome/federica1992/Sim_Geant/Builds/JobsOutput/NUSESprotonPow_0.1-200.25_onAxis/rootOutput/NUSESprotonPow_0.1-20-0.25_onAxis_1000000-evt 0.25

if __name__ == "__main__":

    mainDir = "/lustrehome/federica1992/Sim_Geant/Analysis/"
    #InputFile
    try:
    ...
    BigClusterTree    = TTree("BigClusterTree", "BigClusterTree")#not used
...
#reading the simulation outfile#
    ...
    for i in range(nhit):
        ...
        if np.shape(x)[0] == totLayers and np.shape(y)[0] == totLayers: 
            ...
            x_reco.GetXaxis().SetTitle("zx [mm]")
            x_reco.GetYaxis().SetTitle("x [mm]")
            x_reco.SetTitle("Hor Reconstructed")
            x_true = TGraph(3,zx,xtrack)  
#            print('zx ',zx,' xtrack ',xtrack)
#            print('zx ',zy,' xtrack ',ytrack)
            x_true.SetTitle("Hor MC True")           
            x_true.GetXaxis().SetTitle("zx [mm]")
            x_true.GetYaxis().SetTitle("x [mm]")
            y_reco = TGraphErrors(totLayers,zy,y,dzy,dy)
            y_reco.SetTitle("Ver Reconstructed")
            y_reco.GetXaxis().SetTitle("zy [mm]")
            y_reco.GetYaxis().SetTitle("y [mm]")
            y_true = TGraph(3,zy,ytrack)  
            y_true.SetTitle("Ver MC True")
            y_true.GetXaxis().SetTitle("zy [mm]")
            y_true.GetYaxis().SetTitle("y [mm]")
