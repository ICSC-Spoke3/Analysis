...
if __name__ == '__main__':
    ...
    mainDir = "/lustrehome/federica1992/Nuses/Analysis/"

    #InputFile
    try:
        fin = sys.argv[1] #ClusterTree
        #fin     = "/lustrehome/llorusso/Sim_Geant/Results/Tree/Tree_NUSESe-Pow_0.1-5000_1000000-evt.root"
    ...
    ###Nadia
    pitch = 0.25#((fin.split("/")[-1]).split(".root")[0]).split("pitch")[-1]
    print("[INFO]Pitch strip =", pitch, " mm")
    OutputName = "Results/Res"+(fin.split("/")[-1].replace("Tree_",""))
    ...
    particle ='mu-'# str((((fgeo.split("/")[-1]).split(".txt")[0]).split("Pow")[0]).split("NUSES")[-1])
    ...
    rfiber  = FibRadius #mm
    pitch   = float(pitch)
    ekmax   = 100000#(((fgeo.split("/")[-1]).split(".txt")[0]).split("Pow")[-1]).split("-")[1]
